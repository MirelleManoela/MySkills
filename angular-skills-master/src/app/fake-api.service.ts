...
export class FakeApiService implements InMemoryDbService {
  ...
  post(reqInfo: RequestInfo) {
    const { collectionName, id, collection, utils } = reqInfo as unknown as { 
    ...
    if (collectionName === 'skills' && id) {
      const skill = utils.findById(collection, id);
      if (skill && skill.likes !== undefined) {
        skill.likes += 1;
